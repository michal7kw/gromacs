...
GROMACS since the initial version 2016 to fix
known issues. It also incorporates all fixes made in version 5.1.4.
...
decomposition. Using the type index for the restraint and a
domain-local, but not thread-local index for the pair resolves these
issues. The are now only two limitations left:
...
Ewald surface and 3DC correction forces were only applied up to, but not
including, the last atom with exclusions. With water at the end of the
system only the last H would not be corrected. With ions at the end all
ions would be missing. In addition, with the Verlet scheme and domain
decomposition no force correction was applied at all.
...
eventually incorrectly placed at 0/0/0 due to a memory error when
referencing to rpos\[XX\]\[mol\].
...
which makes runs non-reproducible (but still fully correct). Now thread
local buffers are used. Also removed OpenMP parallezation for small
count (e.g. shift forces).
...
(which is safe on non-512 now, works on KNL, and is presumed useful for
future AVX512 architectures).
...
again without DLB. If without DLB the cycle count is still lower, we
keep DLB off for the remainder of the run. Otherwise is can turn on
again as before. This procedure ensures that the performance will never
deteriorate due to DLB.

## Improved the accuracy of timing for dynamic load balancing with GPUs

With OpenCL, the time for the local non-bonded to finish on the GPU was
ignored in the dynamic load balancing. This change lets OpenCL take the
same code path as CUDA.
...
This corrects our initial guess of kernel tuning parameters that
resulted in reduced occupancy on sm_60 GPU, and thus improves
performance.
...
`gmx wham` did not pick the reaction coordinate from `pullx.xvg` if the
COM of the pull groups were written. `gmx wham` was tested with various
pull options and geometries.
...
This tool now always reports the file and line in user input files that
lead to a condition such that subsequent parsing cannot continue.
...
Depending on the accuracy of the floating point division, the input of
the test function could be 1ulp too large or too small. If it was too
large the result of the test function wasn't within 4ulp and the test
failed.
...
The available wallcycle counters and subcounters reported in the md.log
files are now listed and and explained in the user guide, along with how
to enable reporting of the subcounters.
...
decimal places for x, v and box respectively to ensure compatibility
with other software. Variable-precision reading is still supported.

`2037`

## Fixed BG/Q platform files and install guide

Renamed the platform file to reflect normal practice and the install
guide.

## Reduced the memory required for free-energy simulations

Pair lists with atoms whose short-ranged parameters are perturbed now
use less memory.
