...
Note that although this (slightly) changes the reported averages, the
averages were not incorrect.
...
would not check if nstexpanded was a multiple of nstcalcenergy. If the
latter was not the case, results might have been incorrect.

`2714`

## Issue with do_dssp and unknown residues

The do_dssp tool would fail with unknown residues, as well as have
issues on Windows.
