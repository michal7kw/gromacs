...
> - User supplied tables for short-range nonbonded interactions are not
>   supported.
> - Switched short-range nonbonded interactions with PME are not
>   supported.
...
The gmx anadock tool was removed since it does not belong in
GROMACS (it analyzes AutoDock outputs).

## gmx dyndom

The gmx dyndom tool was removed since it does not belong in
GROMACS (it analyzes DynDom outputs).
...
This feature sometimes overrode the effects of various .mdp settings in
a way that was difficult to understand and report. A user who wants to
do communication between PP ranks less often should choose their `nst*`
