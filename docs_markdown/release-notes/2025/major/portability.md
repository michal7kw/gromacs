...
simulations on AMD devices. For now only the NBNxM kernels are can be
offloaded to the device using this backend.

`4947`

## Added support for the oneMath interface library for GPU FFTs

This enables cross-vendor support for GPU FFTs to the
GROMACS SYCL backend. Either cuFFT or rocFFT
can now be used with Intel DPC++ and Codeplay's plugins for NVIDIA and
AMD GPUs.
