...
(sec. `pbc`) and the *group concept* (sec. `groupconcept`). The MD
algorithm is described in sec. `MD`: first a global form of the
algorithm is given, which is refined in subsequent subsections. The
(simple) EM (Energy Minimization) algorithm is described in sec. `EM`.
Some other algorithms for special purpose dynamics are described after
this.
...
of forces is given in chapter `ff`. In the present chapter we describe
other aspects of the algorithm, such as pair list generation, update of
velocities and positions, coupling to external temperature and pressure,
conservation of constraints. The *analysis* of the data generated by an
MD simulation is treated in chapter `analysis`.
