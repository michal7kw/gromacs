...
In chapter `algorithms`, it was explained how *groups of atoms* can be
used in `mdrun ` (see sec. `groupconcept`). In most analysis
...
radial distribution function (RDF) $g_{AB}(r)$ of A with respect to B,
...
where $V$ is the volume and $P(r)$ is the probability of finding a B
atom at distance $r$ from an A atom.

By having the user define the *atom numbers* for groups A and B in a
simple file, we can calculate this $g_{AB}$ in the most general way,
without having to make any assumptions in the RDF program about the type
of particles.
...
`index file <ndx>` `index.ndx`), there are a couple of programs to
generate them, using either your input configuration or the topology. To
generate an index file consisting of a series of *atom numbers* (as in
the example of $g_{AB}$), use `gmx make_ndx` or `gmx select`. To
generate an index file with angles or dihedrals, use `gmx mk_angndx`. Of
course you can also make them by hand. The general format is presented
here:
...
line, rather than interactively. Consult our [webpage]() for
suggestions.

## Default Groups

When no index file is supplied to analysis tools or
`grompp `, a number of default groups are generated to
choose from:
...
`MainChain`  
protein main chain atoms: N, C$_{\alpha}$, C and O, including oxygens
in C-terminus
...
`SideChain`  
protein side chain atoms; that is all atoms except N, C$_{\alpha}$, C,
O, backbone amide hydrogens, oxygens in C-terminus and hydrogens on the
N-terminus
...
constructions of NH$_3$ groups and tryptophan side-chains), see also
sec. `vsitetop`; this group is only included when it differs from the
`Protein` group
...
`molecule_name`  
for all residues/molecules which are not recognized as protein, DNA, or
RNA; one group per residue/molecule name is generated
...
*(dynamic) selections*. There are three main differences to traditional
index groups:
...
`within 2 of resname DEF` selects all atoms in residues named ABC that
are within 2nm of any atom in a residue named DEF.
...
selections. The tool provides, among others, output options `-on` and
`-ofpdb` to write out the selected atoms to an index file and to a `pdb`
file, respectively. This does not allow testing selections that evaluate
to center-of-mass positions, but other selections can be tested and the
result examined.
...
any selection-enabled tool, as well as with `gmx help `
selections. The help is divided into subtopics that can be accessed
with, e.g., `help syntax`/ `gmx help ` `selections syntax`.
Some individual selection keywords have extended help as well, which can
be accessed with, e.g., `help keywords` within.
...
`gmx select ` -on to generate static index groups to pass to
the tool. However, this only allows for a small subset (only the first
...
of the flexibility of these selections: see the `template.cpp` file in
the `share/gromacs/template` directory of your installation for an
example and
